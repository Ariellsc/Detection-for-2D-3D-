...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(3, self.in_channel, kernel_size=7, stride=2,
                               padding=3, bias=False)  # 使用BN的时候是不需要加入偏置的
        ...
        if self.include_top:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # output size = (1,1)：自适应的平均池化下采样，无论你的输入特征矩阵维度如何，输出维度都将为[1, 1]
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        if stride != 1 or self.in_channel != channel * block.expansion:
            # 这里的下采样结构downsample也就是论文中虚线残差结构部分的1*1的卷积结构。
            downsample = nn.Sequential(
                nn.Conv2d(self.in_channel, channel * block.expansion, kernel_size=1, stride=stride, bias=False),  # 使用BN的时候是不需要加入偏置的。
    ...
    def forward(self, x):
        ...
        if self.include_top:
            x = self.avgpool(x)
            x = torch.flatten(x, 1)  # tensor展平，按照第一维度channel展平
            x = self.fc(x)
